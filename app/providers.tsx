...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // 🌟 OUR NEW CUSTOM NOTIFICATION STATE 🌟
  const [notification, setNotification] = useState<string | null>(null);

  // Helper to show notification and auto-hide after 3 seconds
  ...
  useEffect(() => {
    ...
    const loadDatabase = async () => {
        ...
        const { data: profileData } = await supabase.from('seller_profile').select('*').eq('id', 1).single();
  ...
  // --- ACTIONS (Now using custom showNotification) ---
  ...
  return (
    <CartContext.Provider value={{ 
      ...
      {/* 🌟 100% BULLETPROOF CUSTOM NOTIFICATION 🌟 */}
      {notification && (
